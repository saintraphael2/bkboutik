...
use App\DataTables\EtatDepotDataTable;
...
class EtatDepotController extends AppBaseController
{
     ...
     */
    public function index(EtatDepotDataTable $etatDepotDataTable, Request $request)
    {
        $caissiers = User::get();
        $query = Boutique::orderby('id','desc');
        if(Auth::user()->comptable!=1){
            $caissiers = User::where('id',Auth::user()->id)->get();
             $etatDepotDataTable->caissier = Auth::user()->id;
            $query = $query->where('caissier', Auth::user()->id);
        }
        
         $etatDepotDataTable->comptable=Auth::user()->comptable;
        if($request->caissier){
             $etatDepotDataTable->caissier = $request->caissier;
        ...
        if($request->fromDate && $request->toDate){
            ...
            $toDate = Carbon::parse($request->toDate)->endOfDay();
             ...
             $etatDepotDataTable->fromDate = $fromDate;
             $etatDepotDataTable->toDate = $toDate;
        ...
        $caisse = $query->sum('ttc');
         ...
         $etatDepotDataTable->comptable=Auth::user()->comptable;
        return  $etatDepotDataTable->render('etatDepots.index',[
