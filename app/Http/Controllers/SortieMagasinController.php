...
use App\DataTables\SortieMagasinDataTable;
...
class SortieMagasinController extends AppBaseController
{
     ...
     */
    public function index(SortieMagasinDataTable $sortieMagasinDataTable)
    {
    return $sortieMagasinDataTable->render('sortie_magasin.index');
    }
   ...
   Storage::disk('uploads')->put('caisse/'.$filename, $pdfRecu->output());
    ...
    {
        $boutique = $this->boutiqueRepository->find($id);
        $detailsBoutique=DetailBoutique::where('boutique',$id)->get();
        ...
        return view('sortie_magasin.show')->with(['boutique'=> $boutique,'detailsBoutique'=>$detailsBoutique]);
    }
