...
@section('content')
    ...
    <div class="content px-3">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    @include('sortie_magasinEnt.show_fields')
                </div>
            </div>
        </div>
    </div>
    ...
    <input type="hidden" name="vente" value="{{$vente->id}}">
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    <?php $compteur=1; ?>
                    @foreach($detailsVente as $detailVente)
                        <tr>
                            <td style="border: 1px solid black;"> {{$compteur++;}} </td>
                            <td style="border: 1px solid black;">{{ $detailVente->produitBoutique->code}}</td>
                            <td style="border: 1px solid black;">{{$detailVente->produitBoutique->libelle}}</td>
                            <td style="border: 1px solid black; text-align:right" >{{$detailVente->quantite}}</td>
